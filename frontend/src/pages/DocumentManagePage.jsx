...
function DocumentManagePage() {
  ...
  const searchUsers = async () => {
    ...
    httpService.get(`/users/search/${searchQuery}`)
      .then(res => {
        ...
        const filtered = res.data.filter(u => !existingUserIds.includes(u.id))
  ...
  const addAccess = async (userId, accessLevel) => {
    ...
    try {
      await httpService.post(`/document/access`, {
        userId,
        accessLevel,
  ...
  const updateAccess = async (accessId, accessLevel) => {
    try {
      await httpService.put(`/document/${documentId}/access/${accessId}`, {
        accessLevel
      })
  ...
  const removeAccess = async (accessId, username) => {
    ...
    try {
      await httpService.delete(`/document/access/${accessId}`)
